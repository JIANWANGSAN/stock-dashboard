...
- **竞价判定（并入候选池）**：候选池「竞价硬线」「竞价结论」两列。口径 `竞价额 ≥ 该票「上板分时量 × 50%」` 为**合格**；硬线字段 `bid_required_yi`（仅前 5 只有真分时数据，其余回退 `全天成交额×50%`）；判定结果写回每条候选 `auction_amt_yi` / `auction_ok` / `auction_open` / `auction_pct`，`auction_is_live` 标记是否为真竞价口径。
