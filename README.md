...
- **板块K线（板块名可点击）**：模块3「板块涨幅榜/累计榜」的**板块名可点**，弹出**日K / 周K + MA5/MA10/MA20**（数据源：东财板块指数 `secid=90.BKxxxx`，klt=101日/102周）。板块代码由 `em_boards.py` 在盘后把腾讯板块名映射为东财板块代码后写入 `board_daily[].em_code` / `board_3d[].em_code`。
  - ⚠️ **只有能映射到东财板块的才能点**（实测 13/25）：像 靶材/触摸屏概念/陶瓷基板/一汽系/中国建材集团/口含烟 这些**东财根本没有同名板块**（连搜索接口都返回空），故保持纯文本、不可点。
