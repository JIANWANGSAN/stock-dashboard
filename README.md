...
- **竞价核对口径**：`竞价额 ≥ 该票「上板分时量 × 50%」` 为**合格**；硬线字段 `bid_required_yi`（仅前 5 只有真分时数据，其余回退 `全天成交额×50%`）。⚠️ **只有 9:15–9:30 抓到的量额才是竞价口径**（`auction.is_auction=true`），其他时段拿到的是全日成交额、不可比 —— 卡片会如实标注。数据源：东财 `push2` 快照（f46今开/f47量/f48额/f60昨收），腾讯 `qt.gtimg.cn` 兜底。
