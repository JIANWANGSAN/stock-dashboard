...
| `premarket.py` | **盘前任务（每交易日 9:26）**：重算候选池+次日首选 → 抓竞价快照 → 写 `candidates`/`recommend`/`auction` |
| `auction_check.py` | 竞价定格核对：抓候选票竞价快照，比对硬线（上板分时量×50%），被 premarket.py 调用 |
...
- **盘前 / 盘后分工（重要）**：「梯队」菜单顶部三块（**竞价核对 / 我的想法·次日连板推荐 / 连板候选池**）属**盘前内容**，由 `premarket.py` 在每交易日 **9:26**（竞价 9:25 定格后）刷新；盘后 `fetch_data.py` **不再重算**这三块，只沿用上一版（`enrich_tags.py` 也不再重算推荐卡）。其余全部内容（指数/量能/新闻/板块/梯队/节点/模块4）仍是 **15:05 盘后**刷新。三块数据天然互通：同一批 `candidates`，`auction` 直接读它们的 `bid_required_yi` 做合格判定。
- **竞价核对口径**：`竞价额 ≥ 该票「上板分时量 × 50%」` 为**合格**；硬线字段 `bid_required_yi`（仅前 5 只有真分时数据，其余回退 `全天成交额×50%`）。
...
- **竞价实时刷新**：看板在 **9:15–9:32** 且页面打开时，每 **10 秒**直连快照接口刷新一次（前端实时），其余时段显示「定格」结果。
