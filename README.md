...
- **个股点击看行情**：页面股票名（节点票/触发股/首选/候选池/模块4）带 `data-scode`，点击弹层展示 **当日分时 / 日K线**（东财 `push2his` 接口，前端直连——该接口对本站已返回 `Access-Control-Allow-Origin`，故无需后端代理）。模块3 板块「领涨股」只有名称无代码，暂不可点
