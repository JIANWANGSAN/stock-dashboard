...
| `fetch_data.py` | 主采集：指数、新闻、板块榜、连板梯队、三类节点、连板候选、沪深京量能 |
...
- **沪深京量能**（必看置顶柱状图）：`量能 = 上证综指 + 深证综指 + 北证50 的日成交额`，取近 15 个**已收盘**交易日（东财 push2his 日K `f57`，单节点+慢节奏重试）；柱色 红=较昨日放量 / 绿=缩量，橙色虚线=3万亿；盘中当日不纳入（避免不完整量能）。数据字段 `data.js → market_volume = {list:[{date,amount_yi}], date, total_yi}`
